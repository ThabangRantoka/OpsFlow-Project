...
<h1>R 
<?php echo number_format($monthlyPayroll,0,'.',' ');
...
<span>
<i class="legend-dot legend-planning">
</i>Planning</span>
<span>
<i class="legend-dot legend-progress">
</i>In Progress</span>
<span>
...
<div>
<h2 style="margin:0">Project delivery</h2>
</div>
<a href="../Projects/index.php" class="btn btn-light">View all</a>
</div>

<?php if(!$projects){
 
?><div class="empty-state">
<strong>No projects yet</strong>Create a project to see delivery progress here.</div>
<?php }
 else {
...
<div>
<div style="font-weight:600;color:#17263a">
<?php echo htmlspecialchars($p['project_name']);
 
?></div>
<div class="muted small">
<?php echo htmlspecialchars($p['project_code']);
 
?> · due 
<?php echo $p['end_date'] ? date('d M Y',strtotime($p['end_date'])) : '—';
 
?> · R 
<?php echo number_format((float)$p['budget'],0,'.',' ');
 
?></div>
</div>
<span class="pill 
<?php echo strtolower(str_replace(' ','-',$p['status']));
...
<?php echo max(0,min(100,(int)$p['progress']));
...
<?php }
 ...
 $cls=$l['status']==='Approved'?'approved':($l['status']==='Rejected'?'rejected':'pending');
 
?><div style="display:flex;justify-content:space-between;gap:10px;padding:10px 0;border-bottom:1px solid #edf0f4">
<div>
<?php echo htmlspecialchars($l['fullname']);
...
<div style="font-size:13px;color:#26384c">
...
<div class="dept-icon" style="width:40px;height:40px">
<i class="fa-solid fa-building">
</i>
</div>
<div>
<strong style="color:#17263a">
<?php echo $totalDepartments;
 
?> departments</strong>
<br>
<a href="../Departments/index.php" class="muted" style="text-decoration:none">Manage structure</a>
</div>
</div>
</div>
...
<script>
const deptLabels=
<?php echo json_encode($deptLabels);
 
?>, deptValues=
<?php echo json_encode($deptValues);
...
new Chart(document.getElementById('projectChart'),{type:'doughnut',data:{labels:['Planning','In Progress','Completed'],datasets:[{data:[
<?php echo $planningProjects;
 
?>,
<?php echo $inProgressProjects;
 
?>,
<?php echo $completedProjects;
 
?>],backgroundColor:['#177dab','#2ba4a5','#249b65'],borderWidth:2,borderColor:'#fff'}]},options:{responsive:true,maintainAspectRatio:false,cutout:'58%',plugins:{legend:{display:false}}}});
</script>
