...
if ($fullname === "" || $username === "" || !filter_var($email, FILTER_VALIDATE_EMAIL) || $passwordPlain === "" || $passwordPlain !== $confirm || !in_array($role,["Admin","Manager","Employee"],true) || !in_array($status,["Active","Inactive"],true)) {
    ...
    header("Location:add.php?msg=" . urlencode("Please complete the user form correctly."));
 exit();
...
if ($check->get_result()->num_rows) {
    ...
    header("Location:add.php?msg=" . urlencode("Email already exists."));
 exit();
...
header("Location:add.php?msg=" . urlencode("Unable to create user: " . $conn->error));
 exit();
